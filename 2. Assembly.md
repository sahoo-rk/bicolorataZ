...
The aim is to assemble the genome using long reads. Hence, [Flye](https://github.com/fenderglass/Flye) is used to assemble the contigs from the read files. Alternatively [Canu](https://github.com/marbl/canu) can also be used for long read based assembly.
```bash
...
The resulting contigs are furhter stitched together to prepare the extended contigs with the help of evidences from the corrected long reads. We use [LongStitch](https://github.com/bcgsc/LongStitch) for the purpose.
```bash
...
Next, the polishing and consensus calling of the long contigs are performed with the help of the error corrected long reads using [medaka](https://github.com/nanoporetech/medaka).
```bash
# Running medaka
...
Subsequent polishing is performed using with short reads by [Polca](https://github.com/alekseyzimin/masurca), a program distributed as a part of MaSuRCA genome assembly toolkit.
```bash
# Running polca
...
Subsequently, the polca corrected contigs are screened for contaminants using [kraken2](https://github.com/DerrickWood/kraken2) and the identified contaminants are removed using [KrakenTools](https://github.com/jenniferlu717/KrakenTools).
```bash
# Running kraken
...
```
> As a result, the draft assembly `draft.assembly.fa` is recovered.

Subsequently, the mitochondrial contigs within the draft assembly are identified using [GetOrganelle](https://github.com/Kinggerm/GetOrganelle).
```bash
...
```
> Only one contig in the draft assembly `draft.assembly.fa` is recovered as the mitocondrial fragment `mito.draft.fa`.

The draft assemblies are further assessed as mentioned in the post-assembly and mitogenome sections.
