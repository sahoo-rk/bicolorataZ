...
./IUPACpal -f ~/path/to/dir/in/mitochondrion.fa
 ```
In order to identify potential insertion of mitochondrial fragments within nuclear genome (NUMTs), the nuclear genome was searched for mitochondrial signature using a BLAST search. The resulting BLAST output was filted with the help of a in-house script to retrieve the potential NUMTs.
```bash
